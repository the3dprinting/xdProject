#include "xjslcmodel.h"
#include <QFile>
#include <QDataStream>
#include <QDebug>
#include "../xdlib/generate_line.h"
#include "../xdlib/point.h"
...
const xjPoint SLCModel::center() const
{
    Boundary boundary (this->boundary ());
    xjPoint center (boundary.maxX () + boundary.minX (),
...
const xjPoint SLCModel::dimension() const
{
    Boundary boundary (this->boundary ());
    return xjPoint (qAbs (boundary.maxX () - boundary.minX ()),
        ...
        while(!dataStream.atEnd() && dataStream.status () == QDataStream::Status::Ok)
        {
            ...
            for( quint32 boundaryId = 0; boundaryId < numberOfBoundary ; boundaryId++ )
            {
                ...
                for( quint32 verticeId = 0; verticeId <numberOfVertices; verticeId ++)
                {
                    ...
                    xjPoint point(verticeX * unitScale, verticsY * unitScale, minZlevel * unitScale);
                    polygon.append( point );
                }
                if (isSupport)
                    polygon.setType (xjPolygon::PolygonType::Support);
...
void SLCModel::readxdlib (std::vector<float> z,std::vector<xd::ExPolygons>* layers)
{
     for(int i=0 ; i!=z.size() ; ++i)
     {
        ...
        for(xd::ExPolygons::iterator k=layers->operator [](i).begin() ; k!=layers->operator [](i).end() ; ++k)
        {
            ...
            {
                h->make_clockwise();   //内轮廓顺时针
                polygon.clear();
                for(xd::Points::const_iterator hp=h->points.begin() ; hp!=h->points.end() ; ++hp)
                {
...
void SLCModel::skin_core_infill(int interval_num,float space,float shrinkDistance,float angle_start,float angle_delta,qreal LaserPower,qreal ScanSpeed)   //测试用函数
{
    ...
    {
        for(const xjPolygon & P : L)
        {
            if(P.type()== xjPolygon::PolygonType::Contour)
            {
                startIndex=this->indexOf(L);
                flag=true;
                break;
            }
            if(P.type()== xjPolygon::PolygonType::Infill)
                return;   //说明已经填充过了
        }
        if(flag)
            break;
    ...
    for(int i=startIndex; i < this->size()-interval_num + 1 ; i+=interval_num)  //这里必须是<符号！
    {
        ...
        for (int j = i; j != interval_num + i; ++j)
        {
            ...
            for(const xjPolygon & P : this->operator [](j))
            {
                if((P.type()== xjPolygon::PolygonType::Contour)&&(!P.empty()))
                {
                    xd::outline temOutline;
                    for(const xjPoint & D : P)
                    {
        ...
        //第三步：将输出路径转换成Point类型并加入
         for (int j = i; j != interval_num + i; ++j)
         {
            ...
            {
                xjPolygon temP;
                temP.setType(xjPolygon::PolygonType::Infill);
                for(const xd::xdpoint & xp : xo.first)
                {
                    temP.append(xjPoint(xp.x,xp.y,this->layerHeights.operator [](j)));
                }
...
void SLCModel::line_infill(qint8 ContourCount, qreal ContourWidth,float space,float shrinkDistance,float angle_start,float angle_delta,qreal LaserPower,qreal ScanSpeed)   //测试用函数
{
    ...
    for(int i=0 ; i!= this->size();++i)
    {
        ...
        {
            if(this->operator [](i)[j].type()== xjPolygon::PolygonType::Infill)
            {
