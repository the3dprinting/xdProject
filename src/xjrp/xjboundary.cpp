...
Boundary::Boundary () :
    m_min (xjPoint (INFINITY, INFINITY, INFINITY)), m_max (xjPoint (-INFINITY, -INFINITY, -INFINITY))
{}

Boundary::Boundary (const xjPoint &initial)
{
    if (initial.isValid ())
    {
...
{
    stream << XJRP::xjPoint(boundary.minX(), boundary.minY(), boundary.minZ());
    stream << XJRP::xjPoint(boundary.maxX(), boundary.maxY(), boundary.maxZ());
    return stream;
}

QDataStream &operator >>(QDataStream &stream, XJRP::Boundary &boundary)
{
    XJRP::xjPoint min, max;
    stream >> min;
    stream >> max;
