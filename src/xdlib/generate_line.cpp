...
void OutlineToClipperPath(const outline & input,ClipperLib::Path * output)
{
	...
	for(int i=0;i!=input.size();++i)
	{
        output->push_back(ClipperLib::IntPoint(input[i].x/SCALING_FACTOR,input[i].y/SCALING_FACTOR));
	}
}
	...
	if(!TheOutline.empty())  //首先必须保证有数据
	{
		...
		if(lunkuo!=0)
		{
			...
			for (int i=0;i!=TheOutline.size();++i)
			{
				...
				for (int j=0;j!=TheOutline[i].size();++j)
				{
                    temP<<ClipperLib::IntPoint(TheOutline[i][j].x/SCALING_FACTOR,TheOutline[i][j].y/SCALING_FACTOR);
				}
			...
			for(int temi=0;temi!=lunkuo;++temi)
			{
                ...
                temO.Execute(solution, -offsetWidth/SCALING_FACTOR*(temi+1));
...
{	
	...
	{	
		...
		for (int j=0;j!=theOutline[i].size();++j)
		{
			...
			{
                temP<<ClipperLib::IntPoint(theOutline[i][j][k].x/SCALING_FACTOR,theOutline[i][j][k].y/SCALING_FACTOR);
			}
		...
		for (int j=0;j!=theOutline[i+1].size();++j)
		{
			...
			{
                temP<<ClipperLib::IntPoint(theOutline[i+1][j][k].x/SCALING_FACTOR,theOutline[i+1][j][k].y/SCALING_FACTOR);
			}
			clip.push_back(temP);
		}
...
void OutlinesOffsetMethod(std::vector<xd::outlines> theOutline,std::vector<xd::outputOutlines> &result,float width,float shrinkDistance)
{
	...
	for (int i=0;i!=N;++i)
	{
		...
		for (int j=0;j!=theOutline[i].size();++j)
		{
			...
			for (int k=0;k!=theOutline[i][j].size();++k)
			{
                temPath<<ClipperLib::IntPoint(theOutline[i][j][k].x/SCALING_FACTOR,theOutline[i][j][k].y/SCALING_FACTOR);
			}
	...
	if (!offsetResult.empty())
	{
		for (int i=0;i!=offsetResult.size();++i)
		{
			...
			for (int j=0;j!=offsetResult[i].size();++j)
			{
                ...
                temOutline.push_back(xd::xdpoint(offsetResult[i][j][0].X*SCALING_FACTOR,offsetResult[i][j][0].Y*SCALING_FACTOR));  //加上最后一个点
...
//		}
//		temOutline.push_back(xd::xdpoint(areaA[i][0].X*SCALING_FACTOR,areaA[i][0].Y*SCALING_FACTOR));   //加上最后一个点，保证封闭
//		temPair.first=temOutline;
//		temPair.second=N;
//		temOutputOutlines.push_back(temPair);
//	}
	...
	for (int i=0;i!=areaA.size();++i)  //需要使用多层填充加入区域A的填充路径  需要先分开，再填充！
	{
        ...
        temPush.push_back(xd::xdpoint(areaA[i][0].X*SCALING_FACTOR,areaA[i][0].Y*SCALING_FACTOR));  //加上最后一个点，保证封闭
...
void SplitMNArea(outlines TheOutline,std::vector<xd::outlines> & theResult,std::vector<outlines> & dataOffsets,float width,int M,int N,int lunkuo,float overlap)
{
	if(!TheOutline.empty())  //首先必须保证有数据
	{
		...
		if(lunkuo!=0)
		{
			...
			{
				ClipperLib::Paths solution; 		
                temO.Execute(solution, -width/SCALING_FACTOR*(temi+1));
		...
		{			
			...
			for(int i=0;i!=TheOutline.size();++i)
			{
				...
				for(int j=0;j!=TheOutline[i].size();++j)
				{
                    temPath<<ClipperLib::IntPoint(TheOutline[i][j].x/SCALING_FACTOR,TheOutline[i][j].y/SCALING_FACTOR);
				}
            ...
            if(((AABB.bottom-AABB.top)<10/SCALING_FACTOR)||(AABB.right-AABB.left<10/SCALING_FACTOR))  //如果外包围盒长或宽小于10mm，则不划分了！此处可以修改
			{
			...
			for(int i=0;i!=M;++i)	
			{
				for(int j=0;j!=N;++j)
				{
					...
					for(int k=0;k!=intersectionResult.size();++k)
					{
                        ...
                        temOutline.push_back(xd::xdpoint(intersectionResult[k][0].X*SCALING_FACTOR,intersectionResult[k][0].Y*SCALING_FACTOR)); //加上最后一个点封闭
...
int SplitLWArea(outlines TheOutline, std::vector<xd::outlines> & theResult, std::vector<outlines> & dataOffsets, float width, int Length, int Width, int lunkuo, float overlap, float threshold)
{
	if(!TheOutline.empty())  //首先必须保证有数据
	{
		...
		{			
            ...
            ClipperLib::cInt lineUnit=Width/SCALING_FACTOR;  //行单元的大小，注意：clipper库bottom>top! y轴向下！
            ClipperLib::cInt rowUnit=Length/SCALING_FACTOR;   //列单元的大小
			...
			if(lengthRemainder>rowUnit/2||N==0)
				N+=1;*/
            if (widthRemainder>threshold/SCALING_FACTOR || M == 0)
				M += 1;
            if (lengthRemainder>threshold/SCALING_FACTOR || N == 0)
				N += 1;
			for(int i=0;i!=M;++i)	
			{
				for(int j=0;j!=N;++j)
