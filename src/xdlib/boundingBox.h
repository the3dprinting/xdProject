...
class BoundingBoxBase
{
    ...
    void merge(const PointClass &point);  //!< 加上point(一个点)，并更新min和max，如果开始没有点，则把min和max都设置为point
    void merge(const std::vector<PointClass> &points);  //!< 加上points(一系列点组成的向量)后，更新得到的min和max，实现调用了下面的函数
    void merge(const BoundingBoxBase<PointClass> &bb);  //!< 加上新的bb(包围盒)后，更新min和max
