...
typedef struct stl_hash_edge {
  unsigned       key[6];
  int            facet_number;  //对应面的索引号
  int            which_edge;   //边的索引号
  struct stl_hash_edge  *next;
} stl_hash_edge;

typedef struct {
  int   neighbor[3];  //记录相邻三角面片的索引值
  char  which_vertex_not[3];   //真不容易！这个代表
} stl_neighbors; //这个是加入一个面片的三个相邻面片的索引值
...
typedef struct {
  FILE          *fp;   //文件指针类型
  stl_facet     *facet_start;   //存放所有面
  stl_edge      *edge_start;   //存放所有边
  stl_hash_edge **heads;  //存放所有边，哈希方式
  stl_hash_edge *tail;
  int           M;  //哈希函数的某个参数
  stl_neighbors *neighbors_start;  //存放所有相邻面片的索引值
  v_indices_struct *v_indices;  //存放点的索引
  stl_vertex    *v_shared;  //存放所有共享点
  stl_stats     stats;   //存放所有状态数据
  char          error;
} stl_file;


extern void stl_open(stl_file *stl, char *file);   //在stlinit.c文件里面有读取stl文件的实现过程，可以判定格式binary还是ASCII，有兴趣可以读读！
