...
stl_generate_shared_vertices(stl_file *stl) {
  ...
  stl_invalidate_shared_vertices(stl);  //使得本身的v_indices和v_shared指向NULL
  ...
  stl->stats.shared_malloced = stl->stats.number_of_facets / 2;  //状态中的共享内存分配大小赋值为刚刚分配的大小，即面片数目的一半
  stl->stats.shared_vertices = 0;   //状态中的共享点的数目赋值为零
  ...
  for(i = 0; i < stl->stats.number_of_facets; i++) {
    ...
    for(j = 0; j < 3; j++) {
      ...
      if(stl->stats.shared_vertices == stl->stats.shared_malloced) { //为真说明为共享点分配的共享内存不够了，需要增加
        stl->stats.shared_malloced += 1024;   //一次增加1024个点的位置
        stl->v_shared = (stl_vertex*)realloc(stl->v_shared,
                                             stl->stats.shared_malloced * sizeof(stl_vertex));
        if(stl->v_shared == NULL) perror("stl_generate_shared_vertices");
      }
	  //注意下面这句话，stl->stats.shared_vertices存储着v_shared一共有多少个点，因此，索引代表加入一个新的共享点的位置
      ...
      reversed = 0;
      facet_num = i;
      vnot = (j + 2) % 3;  //这个数一开始不可能>2
	  //下面不太好理解，先假设j=0,则vnot=2
      for(;;) {
        if(vnot > 2) {
          if(direction == 0) {
        ...
        stl->v_indices[facet_num].vertex[pivot_vertex] =
          stl->stats.shared_vertices;   //表明第facet_num个面片的pivot_vertex（中心点）点的索引号等于状态中的shared_vertices
      ...
      stl->stats.shared_vertices += 1;  //这里才表明共享点真正增加了一个
    }
  }
}
